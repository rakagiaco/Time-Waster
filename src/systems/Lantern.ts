...
export class Lantern {
    ...
    public update(): void {
        ...
        if (this.isActive) {
            const flicker = Math.sin(this.flickerTimer * 8) * 0.1 + 0.9; // 0.8 to 1.0
            this.flameSprite.setAlpha(flicker);
        }
    ...
    private createGradientLight(): void {
        ...
        const flicker = Math.sin(this.flickerTimer) * 0.05 + 0.95; // 0.9 to 1.0
        
        // Base light radius - smaller during day, larger at night
        const baseRadius = darknessIntensity > 0.3 ? 80 : 40; // Smaller radius during day
        const currentRadius = baseRadius * flicker;
        
        // Create gradient light effect - stronger at center, weaker at edges
        const gradientSteps = 15;
        
        for (let i = 0; i < gradientSteps; i++) {
            // Calculate radius for this step
            const radiusRatio = (i + 1) / gradientSteps; // 1/15, 2/15, ... 15/15
            ...
            // Scale light intensity based on time of day
            let lightIntensity;
            if (darknessIntensity < 0.1) {
                // Day time - very subtle light (just slightly brighter than surroundings)
                lightIntensity = (1 - distanceFromCenter) * 0.08; // Reduced to 8% intensity during day
            } else if (darknessIntensity < 0.5) {
                // Dawn/Dusk - moderate light
                lightIntensity = (1 - distanceFromCenter) * darknessIntensity * 0.4; // Max 40% of darkness
            } else {
                // Night time - full light effect
                lightIntensity = (1 - distanceFromCenter) * darknessIntensity * 0.6; // Max 60% of darkness
            }
            
            // Only draw if intensity is significant
            if (lightIntensity > 0.01 && this.lightEffect) {
